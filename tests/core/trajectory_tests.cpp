import mininav.core.trajectory;

#include <gtest/gtest.h>

#include <utility>

namespace
{
    constexpr double kEps = 1e-12;

    // Trajectory<T> 是通用容器, 不绑定任何具体状态类型。用一个轻量记录类型
    // 驱动测试, 既验证模板性, 又让该测试不依赖被删除的 SimStateV0/V1。
    struct Sample
    {
        double t{0.0};
        double x{0.0};
        double y{0.0};
        double yaw{0.0};
    };
} // namespace

TEST(Trajectory, DefaultIsEmpty)
{
    const mininav::Trajectory<Sample> traj;
    EXPECT_TRUE(traj.empty());
    EXPECT_EQ(traj.size(), 0u);
}

TEST(Trajectory, AppendIncreasesSize)
{
    mininav::Trajectory<Sample> traj;
    traj.append(Sample{.t = 0.0});
    traj.append(Sample{.t = 0.1});
    traj.append(Sample{.t = 0.2});
    EXPECT_EQ(traj.size(), 3u);
    EXPECT_FALSE(traj.empty());
}

TEST(Trajectory, RecordsPreservedInOrder)
{
    mininav::Trajectory<Sample> traj;
    traj.append(Sample{.t = 0.0});
    traj.append(Sample{.t = 1.0, .x = 1.0, .y = 2.0, .yaw = 0.5});
    traj.append(Sample{.t = 2.0, .yaw = 1.0});

    const auto& recs = traj.records();
    ASSERT_EQ(recs.size(), 3u);
    EXPECT_NEAR(recs[0].t, 0.0, kEps);
    EXPECT_NEAR(recs[1].x, 1.0, kEps);
    EXPECT_NEAR(recs[1].y, 2.0, kEps);
    EXPECT_NEAR(recs[2].yaw, 1.0, kEps);
}

TEST(Trajectory, ClearMakesEmpty)
{
    mininav::Trajectory<Sample> traj;
    traj.append(Sample{.t = 0.0});
    traj.append(Sample{.t = 0.1});
    traj.clear();
    EXPECT_TRUE(traj.empty());
    EXPECT_EQ(traj.size(), 0u);
}

TEST(Trajectory, ReserveDoesNotChangeSize)
{
    mininav::Trajectory<Sample> traj;
    traj.reserve(1000);
    EXPECT_TRUE(traj.empty());
    EXPECT_EQ(traj.size(), 0u);
}

TEST(Trajectory, MoveAppend)
{
    mininav::Trajectory<Sample> traj;
    Sample r{.t = 1.0, .x = 1.0, .y = 2.0};
    traj.append(std::move(r));
    ASSERT_EQ(traj.size(), 1u);
    EXPECT_NEAR(traj.records()[0].x, 1.0, kEps);
}

// ---------------------------------------------------------------------------
// 验证 Trajectory<T> 真的是"模板":能用另一种无关数据类型实例化。
// ---------------------------------------------------------------------------
namespace
{
    struct DummyRecord
    {
        double t{0.0};
        int payload{0};
    };
} // namespace

TEST(Trajectory, WorksWithCustomRecordType)
{
    mininav::Trajectory<DummyRecord> traj;
    traj.append(DummyRecord{0.0, 42});
    traj.append(DummyRecord{1.0, 84});
    ASSERT_EQ(traj.size(), 2u);
    EXPECT_EQ(traj.records()[0].payload, 42);
    EXPECT_EQ(traj.records()[1].payload, 84);
}
