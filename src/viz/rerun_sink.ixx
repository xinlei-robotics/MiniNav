...
{
    // ---------------------------------------------------------------------------
    // RerunSink: VizSink 的 Rerun 实现(对 Rerun C++ SDK 的薄封装)。
    // ---------------------------------------------------------------------------
    class RerunSink final : public VizSink
    {
    public:
        ...
        void set_time(double t_seconds) override;

        // ---- 每帧动态数据 ----
        void log_pose(std::string_view entity_path, const Pose2D& pose) override;
        void log_twist(std::string_view entity_path, const Twist2D& twist) override;
        void log_scalar(std::string_view entity_path, double value) override;
        void log_axes(std::string_view entity_path, float length = 0.5F) override;
        void log_axes_static(std::string_view entity_path, float length = 0.5F) override;
